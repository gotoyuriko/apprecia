...
interface CommentProps {
    commentItem: any;
    commentUserData: AppUser | AppUser[] | null;
    status: 'new' | 'old';
    currentUser: User | null;
    artProjectItem: ArtProject;
    setLiveComments: Dispatch<SetStateAction<any[]>> | null;
}
...
export default function Comment({
    ...
    const filteredUser = useMemo(() => {
        if (status === "new") {
            return commentUserData as AppUser;
        } else if (status === "old") {
            return (commentUserData as AppUser[])?.find((userItem) => userItem.user_email === commentItem.comment_user);
        }
    ...
    const handleUpdate = async () => {
        const result = await GetArtwork(artProjectItem.project_creator, artProjectItem.project_createdAt);
        if (result) {
            const updateResult = await UpdateComment(result.id, commentItem, updatedComment);
            if (updateResult) {
                setUpdatedComment(updateResult.newUpdatedComment);
                setDisplayComment(updateResult.newUpdatedComment);
            }
        }
        setIsEditing(false);
    };
    ...
    const handleDelete = async () => {
        const result = await GetArtwork(artProjectItem.project_creator, artProjectItem.project_createdAt);
        if (result) {
            await DeleteComment(result.id, commentItem);
        }

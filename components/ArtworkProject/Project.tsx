...
const Project = ({ currentUser, status, slug }: ProjectProps) => {
    const router = useRouter();
    const [projectData, setProjectData] = useState<Partial<ArtProject>>({
        project_title: "",
        project_description: "",
        project_tags: [],
        project_skills: [],
    ...
    const [artworkData, setArtworkData] = useState<Partial<ArtProject>>(projectData);
        ...
        if (status === "update") {
            const fetchData = async () => {
                try {
                    ...
                    setArtworkData(data as ArtProject);
                } catch (error) {
    ...
    const isButtonDisabled =
        status === 'new' ?
            ...
            (projectData.project_tags?.length ?? 0) === 0 ||
            (projectData.project_skills?.length ?? 0) === 0
            :
            images.length === 0 ||
            !artworkData.project_title ||
            !artworkData.project_description ||
            (artworkData.project_tags?.length ?? 0) === 0 ||
            (artworkData.project_skills?.length ?? 0) === 0;
    ...
    const customStyles = {
        ...
        multiValueRemove: (styles: any, { data }: { data: SelectOption }) => ({
            ...styles,
            color: data.color,
            ":hover": {
    ...
    const uploadProjectData = async () => {
        ...
        if (images !== null && images.length !== 0 && projectLinkValid === true) {
            ...
            const result =
                status === 'new' ?
                    await AddArtwork(images as File[], projectData as any)
                    : await UpdateArtworkFromId(images, artworkData as ArtProject, slug!);
                        ...
                        <CreatableSelect
                            ...
                            onChange={(selectedOptions) => {
                                status === 'new' ?
                                    setProjectData((prevData) => ({
                                        ...prevData,
                                        project_tags: selectedOptions as SelectOption[],
                                    })) :
                                    setArtworkData((prevData) => ({
                                        ...prevData,
                                        project_tags: selectedOptions as SelectOption[],
                        ...
                        <CreatableSelect
                            ...
                            onChange={(selectedOptions) => {
                                status === 'new' ?
                                    setProjectData((prevData) => ({
                                        ...prevData,
                                        project_skills: selectedOptions as SelectOption[],
                                    })) :
                                    setArtworkData((prevData) => ({
                                        ...prevData,
                                        project_skills: selectedOptions as SelectOption[],

...
export default function ProjectModal({
    ...
    // Tooltip
    const [tooltip, setTooltip] = useState<null | HTMLElement>(null);
    const handleTooltip = (event: React.MouseEvent<HTMLElement>) => { setTooltip(event.currentTarget); };
    ...
    const handleEdit = () => {
        const fetchData = async () => {
            ...
            if (result) router.push(`/projects/update/${result.id}`);
        };
        fetchData();
    };
    ...
    const handleDeleteProject = async () => {
        ...
        if (result?.id) {
            try {
                await DeleteArtwork(artProjectItem.project_imageUrls, result.id, artProjectItem.project_creator);
                setOpen(false);
                router.reload();
            } catch (error) {
                ...
                >
                    ...
                    {commentData && setCommentData && (
                        <CommentSection
                            ...
                            usersData={usersData}
                            commentCurrentUserData={commentCurrentUserData ?? null}

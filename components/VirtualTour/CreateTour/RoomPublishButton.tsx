...
interface RoomPublishButtonProps {
    status: 'new' | 'update';
    uid: string;
...
export default function RoomPublishButton({ status, uid, setTourData, tourData, slug }: RoomPublishButtonProps) {
    ...
    const handlePublishProject = () => {
        const result = status === 'new' ? AddArtGallery(tourData) : UpdateArtGallery(tourData, slug!);

        (result as any).then ? (result as Promise<any>).then(({ error }: any) => {
            handlePublishResult(error);
        }) : handlePublishResult((result as any).error);
    };

    const handlePublishResult = (error: unknown) => {
        if (error) {
    ...
    const handleUpdateProject = () => {
        const result = UpdateArtGallery(tourData, slug!);

        (result as any).then ? (result as Promise<any>).then(({ error }: any) => {
            handleUpdateResult(error);
        }) : handleUpdateResult((result as any).error);
    };

    const handleUpdateResult = (error: unknown) => {
        if (error) {
            console.error("Error uploading images or adding document: ", error);
            setUpdateMsg("");
        } else {
    ...
    return (
        <div className="absolute bottom-5 right-5 z-10">
            <div className="flex flex-row items-center">
                ...
                >
                    ...
                    <DialogActions>
                        ...
                        <Button
                            onClick={status === 'new' ? handlePublishProject : handleUpdateProject}
                            variant="contained"
                            autoFocus
                        >
                            {status === 'new' ? 'Publish' : 'Update'}
            ...
            <div className="text-white">{updateMsg}</div>
        </div>
    );
}

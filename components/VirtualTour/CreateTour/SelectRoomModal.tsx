...
export default function SelectRoomModal({
    ...
    const isButtonDisabled = selectedImage === 0 || !tourData?.tour_name || selectedAudio === '';

    const handleAudioSelection = (audioId: number) => {
        if (audioId !== 1) {
            if (audioId === selectedAudio) {
                ...
                const foundAudio = audioList.find((audio) => audio.id === audioId);
                if (foundAudio) {
                    audioRef.current = new Audio(foundAudio.src);

...
export default async function AddComment(uid, createdAt, commentData) {
    // Check if commentData is defined and has the required properties
    if (!commentData || !commentData.comment || !commentData.user_id || !commentData.user_name) {
        console.error("Error: Invalid comment data", commentData);
        return;
    }
    try {
